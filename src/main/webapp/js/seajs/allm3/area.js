define( function( require, exports, module ) {
    ...
    var AJAX_URL = "index.php?/open/getAreaInfo";
	...
	function AreaControl() {
		...
		var generateSelectHtml = function( areaList ) {
			var html = '<select class="form-control area-control-area-select" id="area_id">';
			html += "<option value=''>请选择大区</option>";
			for( var i = 0; i < areaList.length; i++ ) {
				var areaItem = areaList[ i ];
				html += '<option value="'+ areaItem.areaID +'">' + areaItem.areaID+'服_'+ areaItem.areaName + "</option>";
			}
			html += "</select>";
		...
		var generateCheckboxHtml = function( areaList ) {
			...
			for( var i = 0; i < areaList.length; i++ ) {
				...
				html += '<input type="checkbox"  value="'+ areaItem.areaID +'" />' +areaItem.areaID+'服_'+ areaItem.areaName;
				html += '</label>';
			}
			getAreaPlaceholder().html( html )
