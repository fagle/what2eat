/**
 * Description: jQuery.charts.js
 * Modified by: Dingwei
 * Modified contents: 
**/
if(typeof Highcharts === 'undefined'){
	core.includeJs('http://ui.om.dianhun.cn/third/highcharts/js/highcharts.js');
}
...
var Line = function(config){
	...
	this._buildSeries = function(){
		var data = config.data, key = config.key;
	...
	this._buildOption = function(){
		...
		option = {
			...
			markEnabled: config.markEnabled === undefined ? false : true
		};
		return option;
	};
...
var Column = function(config){
	...
	this.series = [];
	this.chart = null;
	this.rank = null; // rank默认为空，显示所有.当要显示1-20，21-40的分页显示的时候才起作用
	...
	this.draw = function(){
		...
		this.option = {
	        ...
	        tooltip: {
	            formatter: function() {
	                return '<b>' + this.x + '</b><br />' + '<b>' + this.series.name + '：' + Highcharts.numberFormat(this.y, 0) + '</b>';
	            }
	        },
	        series: this.series
		};
