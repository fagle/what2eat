...
class InitItemControl{
    ...
    private itemCount = 0;
    private limitCount = Number.MAX_VALUE;

    constructor ( $, templateID, itemList, onAdd ) {
        var self = this;
        this.$ = $;
    ...
    public setLimitCount( value ) {
        this.limitCount = value;
    }

    public getItemListElement() {
...
class Activity{
    static PAGE_MODE_ADD = 'add';
    static PAGE_MODE_EDIT = 'edit';

    static ACTIVITY_TYPE_LEVELING = 1;
    static ACTIVITY_TYPE_RECHARGE = 2;

    //无条件
    static REQUIRE_TYPE_NONE = 0;
    //等级条件
    static REQUIRE_TYPE_LEVEL = 1;
    //单次充值条件
    static REQUIRE_TYPE_ONE_CHARGE = 2;
    //累计充值条件
    static REQUIRE_TYPE_TOTAL_CHARGE = 3;
    //累计元宝消费条件
    static REQUIRE_TYPE_YB_CONSUME = 4;

    private $;
    private itemQuerySuggest: any;

    constructor( $, url: string, pageMode: string, itemQuerySuggest: any ) {
        this.$ = $;
        this.itemQuerySuggest = itemQuerySuggest;
        var level1Count: number = 0;
        var level1 = new InitItemControl( $, 'activity_info_item_tpl', $( '#activity_info_item_placeholder' ), function( itemEl ) {
            level1Count ++;

            var level2Count: number = 0;
    ...
    private getErrorInfo( element, message ) {
        return {
            error: 1,
            element: element,
            message: message
    ...
    public fillForm( activityInfo : any ) {
        var self = this;
        var $ = this.$;
